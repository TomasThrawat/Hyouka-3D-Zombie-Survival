...
class MainActivity : ComponentActivity() {
    ...
    private var lastInteractionUptime: Long = 0L
    ...
    override fun onWindowFocusChanged(hasFocus: Boolean) {
        ...
        if (changed) {
            writeWindowDiagnostics("FOCUS_CHANGED")
            if (!hasFocus && game) {
                window.decorView.postDelayed({ writeFocusLossDiagnostics("FOCUS_LOSS_750MS") }, 750L)
                window.decorView.postDelayed({ writeFocusLossDiagnostics("FOCUS_LOSS_2000MS") }, 2000L)
            }
        }
    }
    ...
    private fun writeFocusLossDiagnostics(reason: String) {
        try {
            val pm = getSystemService(Context.POWER_SERVICE) as PowerManager
            val km = getSystemService(Context.KEYGUARD_SERVICE) as KeyguardManager
            val am = getSystemService(Context.ACTIVITY_SERVICE) as ActivityManager
            val task = am.appTasks.firstOrNull()?.taskInfo
            val top = task?.topActivity
            val root = task?.baseActivity
            writeLog("FOCUS_LOSS_DIAGNOSTICS reason=" + reason +
                " game=" + game +
                ...
                " keyguard=" + km.isKeyguardLocked +
                " finishing=" + isFinishing +
                " changing=" + isChangingConfigurations +
                " destroyed=" + isDestroyed +
                " taskTop=" + top +
                " taskRoot=" + root +
                " sinceInteractionMs=" + (android.os.SystemClock.uptimeMillis() - lastInteractionUptime))
        } catch (t: Throwable) {
            writeLog("FOCUS_LOSS_DIAGNOSTICS_FAILED reason=" + reason + " type=" + t.javaClass.name + "\n" + Log.getStackTraceString(t))
        }
    }
